  ...
  forAll(test) { (name: String, urlPrefix: String, serviceId: String) =>
    s"GET ${urlPrefix}authorize" should {
      "redirect to oauth successfully" in {
        ...
        val response = wsUrl(s"${urlPrefix}authorize?journeyId=journeyId").get().futureValue
        response.status shouldBe 200
      }
    }
   ...
   s"POST ${urlPrefix}token" should {
      def postOAuthToken(form: String): WSResponse = {
        ...
        wsUrl(s"${urlPrefix}token?journeyId=journeyId").addHttpHeaders(jsonHeader).post(parse(form)).futureValue
      }
      ...
      def verifyPostOAuthTokenFailureStatusCode(form: String, upstreamResponseCode: Int, responseCodeToReport: Int): Unit = {
        ...
        val jsonHeader: (String, String) = "Accept" -> "application/vnd.hmrc.1.0+json"
        val response = wsUrl(s"${urlPrefix}token?journeyId=journeyId").addHttpHeaders(jsonHeader).post(parse(form)).futureValue
