  ...
  def oauthRedirectSuccess(service: String): Unit = {
    val redirectUrl = service match {
      case "ngc" =>
        ...
        "response_type=code"
      case "rds" =>
        "/oauth/authorize?client_id=rds_i_whTXqBWq9xj0BqdtJ4b_YaxV8a&redirect_uri=urn:ietf:wg:oauth:2.0:oob:auto&scope=read:mobile-rds-vehicle&response_type=code"
    }
